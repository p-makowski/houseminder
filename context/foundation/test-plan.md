...
| #3 | Volt `markDone` call with a foreign task ID returns 403 and creates no `ServiceRecord` | "The action has an internal guard, but does the Volt component fetch the task through the household scope before passing it to the action?" | How the dashboard Volt component's `markDone()` retrieves the task before passing it to the action | Integration (Volt::test with cross-household task ID) | Testing only the action in isolation; missing the component-level fetch path |
| #4 | Task due exactly today → "overdue"; task due in exactly 7 days → "due this week"; task due in 8 days → "upcoming" | "Happy-path tests use `subDay()` and `addDays(8)` — the exact boundary values are untested" | Whether `<` vs `<=` is used for the date comparisons in the relevant scopes or queries | Integration (Volt::test with `Carbon::setTestNow` at exact boundary timestamps) | Using relative offsets (`subDay()`, `addDays(8)`) in tests instead of pinning to the exact boundary |
